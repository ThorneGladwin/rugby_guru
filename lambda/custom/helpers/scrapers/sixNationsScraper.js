...
const { LIST_OF_SIX_NATIONS_TEAMS, MONTHS, SIX_NATIONS_FIXTURES_URL, REFEREE_NATIONALITIES } = require("../../constants");
let hasHome = false;
let hasAway = false;
let hasDate = false;
let hasScore = false;
let skipNextInstance = false;
let fixture = {};
let allFixtures = [];
...
const processNodes = children => {
  if (children.type === "text" && children.data && (children.data !== "\n" && children.data !== "\n\n")) {
    ...
    if (skipNextInstance && REFEREE_NATIONALITIES.includes(textBlock)) {
      skipNextInstance = false;
    } else {
      if(!skipNextInstance && textBlock.includes("Referee")) {
        skipNextInstance = true;
      }
  
      if (!hasHome || !hasAway) {
        if (LIST_OF_SIX_NATIONS_TEAMS.includes(textBlock)) {
          if (!hasHome) {
            fixture = {
              ...fixture,
              home: textBlock
...
const getSixNationsFixturesForTeam = team => {
  if (team) {
    return rp(SIX_NATIONS_FIXTURES_URL)
    .then(function(html) {
...
const getNextSixNationsFixtureForTeam = team => {
  return new Promise((resolve, reject) => {
    if (team) {
      return getSixNationsFixturesForTeam(team)
        .then(allFixtures => {
...
const getLastSixNationsResultForTeam = team => {
 return new Promise((resolve, reject) => {
  if (team) {
    return getSixNationsFixturesForTeam(team)
      .then(allFixtures => {
...
module.exports = {
  getNextSixNationsFixtureForTeam,
  getLastSixNationsResultForTeam
  };
